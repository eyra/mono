defmodule CoreWeb.Study.New do
  @moduledoc """
  The home screen.
  """
  ...
  def create(socket, changeset) do
    ...
    with {:ok, study} <- Studies.create_study(changeset, current_user),
      ...
      {:ok, Routes.live_path(socket, CoreWeb.Study.Edit, study.id)}
    end
  end
    ...
    ~H"""
      ...
      <ContentArea>
        <Form for={{ @changeset }} submit="create">
          ...
          <SecondaryLiveViewButton label={{ dgettext("eyra-study", "cancel.button") }} event="cancel" />
        </Form>
      </ContentArea>
    """
  end
