...
defmodule LinkWeb.LiveViewPowHelper do
  ...
  defmacro __using__(_opts) do
    ...
    quote do
      ...
      def get_user(socket, session), do: unquote(__MODULE__).get_user(socket, session, @pow_config)
      def assign_current_user(socket, session, user, profile), do: unquote(__MODULE__).assign_current_user(socket, session, user, profile, self(), @renewal_config)
  ...
  def assign_current_user(socket, session, user, profile, pid, renewal_config) do
    maybe_init_session_renewal(socket, pid, session, renewal_config |> Keyword.get(:renew_session), renewal_config |> Keyword.get(:interval))
