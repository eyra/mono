...
|---|---|---|---|---|
| `crew_tasks.status = :accepted` | `40dfa20a3` (2026-08-03) | Backfill `assignment_participation.accepted_at` from the historical task row (use `crew_tasks.updated_at` or the corresponding reward's approval timestamp), then convert lingering `:accepted` task rows to `:completed`. | Task-level accept is retired; the reviewer's decision lives on `assignment_participation.accepted_at`. Old rows still surface via `Crew.Public.rejected_tasks/1`, `Assignment.Public.status/2`, and `Crew.TaskStatus.finished_states/0`. | — |
