defmodule Systems.Graphite.Switch do
  ...
  def intercept({:graphite_tool, _} = signal, %{graphite_tool: tool, from_pid: from_pid} = message) do
    ...
    if leaderboard =
      ...
      dispatch!(
        ...
        Map.put(message, :graphite_leaderboard, leaderboard)
      )
    end
  ...
  @impl true
  def intercept({:graphite_submission, _} = signal, %{graphite_submission: submission} = message) do
    tool = EctoHelper.get_assoc(submission, :tool)

    dispatch!(
      {:graphite_tool, signal},
      Map.put(message, :graphite_tool, tool)
    )
  ...
  defp update_pages(%Graphite.LeaderboardModel{} = leaderboard, from_pid) do
    Enum.each([Graphite.LeaderboardPage, Graphite.LeaderboardContentPage], &update_page(&1, leaderboard, from_pid))
  end
