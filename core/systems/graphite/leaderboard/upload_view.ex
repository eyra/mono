defmodule Systems.Graphite.Leaderboard.UploadView do
  ...
  def update(
    ...
    {:ok,
     ...
     |> assign(
       ...
       replace_button: "replace button",
       csv_local_path: nil,
       csv_remote_file: nil,
       csv_lines: nil
     )
     |> init_file_uploader(:csv)
     |> prepare_process_button("process button")}
  end
  ...
  def handle_event(
        "process",
        _params,
        %{assigns: %{csv_local_path: csv_local_path}} = socket
      ) do
    lines =
      csv_local_path
      |> File.stream!()
    ...
    lines_ok =
      ...
      |> Enum.filter(fn line -> check_line(line, socket.assigns.entity.metrics) end)

    Graphite.Public.import_csv_lines(socket.assigns.entity, lines_ok)

    # FIXME: what to do next? Redirect somewhere?
