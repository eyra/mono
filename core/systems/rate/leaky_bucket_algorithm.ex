defmodule Systems.Rate.LeakyBucketAlgorithm do
  ...
  defp do_request_permission(%State{} = state, quotas, request) do
    quotas
    |> Enum.reduce({:granted, state}, fn quota, acc ->
  ...
  @spec do_request_permission(State.t(), DateTime.t(), String.t(), integer, integer, integer) ::
          Bucket.t()
  ...
  @spec get_or_create_bucket(State.t(), Quota.t(), request, DateTime.t(), String.t()) ::
          Bucket.t()
  ...
  defp bucket_key(
         %Quota{scope: :local, window: window, rate_limit: rate_limit},
         {service, client_id, _}
       ),
       do: "#{rate_limit}/#{window}@#{service}=>#{client_id}"

  @spec bucket_key(Quota.t(), request) :: String.t()
  defp bucket_key(
         %Quota{scope: :global, window: window, rate_limit: rate_limit},
         {service, _, _}
       ),
       do: "#{rate_limit}/#{window}@#{service}"
end
