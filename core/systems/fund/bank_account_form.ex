defmodule Systems.Fund.BankAccountForm do
  ...
  # Successive update
  alias Systems.Fund.BankAccountModel, as: Model
  ...
  def update(%{id: id, bank_account: nil, user: user}, socket) do
    title = dgettext("eyra-fund", "bank.account.create.title")
    bank_account = %Model{}
  ...
  def update(%{id: id, bank_account: bank_account, user: user}, socket) do
    title = dgettext("eyra-fund", "bank.account.edit.title")
    changeset = Model.prepare(bank_account)
    ...
    assign(socket,
      buttons: [
        %{
          ...
          face: %{type: :primary, label: dgettext("eyra-fund", "bank.account.submit.button")}
        },
        %{
  ...
  defp handle_submit(%{assigns: %{bank_account: bank_account}} = socket, attrs) do
    apply_submit(
      socket,
      bank_account |> Model.change(attrs) |> Model.validate() |> Model.submit()
    )
  end
  ...
  def render(assigns) do
    ~H"""
    <div>
      <Text.title3><%= dgettext("eyra-fund", "bank.account.content.title") %></Text.title3>
      ...
      <.text_input form={form}
        field={:name}
        debounce="0"
        label_text={dgettext("eyra-fund", "bank.account.name.label")}
      />
      <.text_input form={form}
        field={:virtual_icon}
        debounce="0"
        maxlength="2"
        label_text={dgettext("eyra-fund", "bank.account.icon.label")}
      />
      <.spacing value="XS" />

      <Text.title3><%= dgettext("eyra-fund", "currency.title") %></Text.title3>
      <.inputs_for :let={currency_form} field={form[:currency]}>
        <.text_input form={currency_form}
          field={:name}
          debounce="0"
          label_text={dgettext("eyra-fund", "currency.name.label")}
        />
        <Text.title6><%= dgettext("eyra-fund", "currency.label.title") %></Text.title6>
        <.spacing value="XS" />
