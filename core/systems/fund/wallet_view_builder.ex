defmodule Systems.Fund.WalletViewBuilder do
  ...
  def view_model(%Bookkeeping.AccountModel{id: id, identifier: ["wallet", currency, _user_id]} = account, user) do
    ...
    currency = Fund.Public.get_currency_by_name(currency, label_bundle: [:items])
    title = Fund.CurrencyModel.title(currency, locale)
    ...
    pending_amount = Fund.Public.pending_rewards(user, currency)
    ...
    earned_label =
      dgettext("eyra-assignment", "earned.label", amount: Fund.CurrencyModel.label(currency, locale, earned_amount))

    pending_label =
      dgettext("eyra-assignment", "pending.label", amount: Fund.CurrencyModel.label(currency, locale, pending_amount))

    togo_label =
      dgettext("eyra-assignment", "togo.label", amount: Fund.CurrencyModel.label(currency, locale, togo_amount))
