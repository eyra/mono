defmodule Systems.Instruction.Switch do
  ...
  @impl true
  def intercept({:instruction_tool, _}, %{instruction_tool: tool, from_pid: from_pid}) do
    update_tool_view(tool, from_pid)
    :ok
  end

  @impl true
  def intercept({:instruction_tool, _}, _message) do
    # Handle case without from_pid
    :ok
  end

  defp update_tool_view(tool, from_pid) do
    dispatch!({:embedded_live_view, Instruction.ToolView}, %{
      id: tool.id,
      model: tool,
      from_pid: from_pid
    })
