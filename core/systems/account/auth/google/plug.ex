defmodule Systems.Account.Auth.Google.PlugUtils do
  def config(otp_app) do
    Application.get_env(otp_app, Systems.Account.Auth.Google)
  end
...
defmodule Systems.Account.Auth.Google.AuthorizePlug do
  import Plug.Conn
  import Systems.Account.Auth.Google.PlugUtils
...
defmodule Systems.Account.Auth.Google.CallbackPlug do
  import Plug.Conn
  import Systems.Account.Auth.Google.PlugUtils
  require Logger
  ...
  defp log_session_not_found(conn) do
    Logger.error("[Systems.Account.Auth.Google] OAuth callback without session state",
  ...
  defp redirect_with_error(conn, error) do
    conn
    |> put_flash(:error, Systems.Account.Auth.SSOHelpers.error_message(error))
