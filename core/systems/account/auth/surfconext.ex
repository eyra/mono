defmodule Systems.Account.Auth.Surfconext do
  @moduledoc """
  SURFconext identity provider. Implements `Systems.Account.Auth.Provider` —
  ...
  def get(%User{id: id}) do
    Repo.get_by(Systems.Account.Auth.Surfconext.UserModel, user_id: id)
  end

  @impl Systems.Account.Auth.Provider
  def attach(%User{} = user, userinfo) when is_map(userinfo) do
    %Systems.Account.Auth.Surfconext.UserModel{}
    |> Systems.Account.Auth.Surfconext.UserModel.register_changeset(satellite_attrs(userinfo))
  ...
  def refresh(%User{} = user, userinfo) when is_map(userinfo) do
    get(user)
    |> Systems.Account.Auth.Surfconext.UserModel.update_changeset(%{userinfo: userinfo})
    |> Repo.update!()
  end
  ...
  defmacro routes(otp_app) do
    quote bind_quoted: [otp_app: otp_app] do
      ...
      scope "/", Systems.Account.Auth.Surfconext do
