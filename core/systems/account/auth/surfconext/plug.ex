defmodule Systems.Account.Auth.Surfconext.PlugUtils do
  alias Systems.Account.Auth

  def config(otp_app) do
    Application.get_env(otp_app, Auth.Surfconext)
  end
...
defmodule Systems.Account.Auth.Surfconext.AuthorizePlug do
  @moduledoc """
  ...
  """
  import Plug.Conn
  import Systems.Account.Auth.Surfconext.PlugUtils
...
defmodule Systems.Account.Auth.Surfconext.CallbackController do
  require Logger
  ...
  defp do_authenticate(conn, params, session_params) do
    ...
    with {:ok, userinfo} <- oidc_module(config).fetch_userinfo(config, token) do
      ...
      case Auth.authenticate_or_transfer(
             Auth.Surfconext,
             userinfo
           ) do
        {:transfer, user} ->
