defmodule Systems.Account.Auth.Email do
  @moduledoc """
  Email-first signup identity provider.

  Creates a provisional Account.User (no password, not confirmed) plus a
  satellite `Systems.Account.Auth.Email.UserModel` record with the UserCheck validation
  result. Mirrors the Systems.Account.Auth.Google / Systems.Account.Auth.Apple / Surfconext pattern.
  ...
  ## Usage
      ...
      case Systems.Account.Auth.Email.register("user@example.com") do
  ...
  5. Creates Account.User + Systems.Account.Auth.Email.UserModel in a single transaction

  ## Options

    * `:policy` - rejection policy module (default: `Systems.Account.Auth.Email.DefaultRejectionPolicy`)
  """
  def register(email, opts \\ []) when is_binary(email) do
    policy = Keyword.get(opts, :policy, Systems.Account.Auth.Email.DefaultRejectionPolicy)
  ...
  @doc """
  Links a real email to an existing user (e.g. one created via Systems.Account.Auth.External
  ...
  3. Calls UserCheck for validation (fails open on timeout/error)
  4. Applies rejection policy
  5. Updates user email + creates Systems.Account.Auth.Email.UserModel satellite
  ...
  @doc """
  Returns the Systems.Account.Auth.Email.UserModel satellite for a given user, or nil.
  """
  def get_by_user(%User{id: user_id}) do
    Repo.get_by(Systems.Account.Auth.Email.UserModel, user_id: user_id)
  end
  ...
  defp validate_email(email, policy) do
    case UserCheck.check_email(email) do
      ...
      {:error, reason} ->
        Logger.warning(
          "[Systems.Account.Auth.Email] UserCheck failed (failing open): #{inspect(reason)}"
        )
  ...
  defp link_email_to_user(%User{id: user_id} = user, email, validation_attrs) do
    ...
    |> Multi.insert(:email_sign_up_user, fn _changes ->
      %Systems.Account.Auth.Email.UserModel{user_id: user_id}
      |> Systems.Account.Auth.Email.UserModel.changeset(validation_attrs)
    end)
    |> Repo.commit()
    |> case do
      {:ok, %{user: user}} ->
  ...
  defp create_user_with_satellite(email, validation_attrs) do
    ...
    |> Multi.insert(:email_sign_up_user, fn %{user: user} ->
      %Systems.Account.Auth.Email.UserModel{user_id: user.id}
      |> Systems.Account.Auth.Email.UserModel.changeset(validation_attrs)
    end)
    |> Repo.commit()
    |> case do
