defmodule Systems.Account.Auth.Apple do
  ...
  def get(%User{id: user_id}),
    do: Repo.get_by(Systems.Account.Auth.Apple.UserModel, user_id: user_id)

  @impl true
  def attach(%User{} = user, userinfo) do
    %Systems.Account.Auth.Apple.UserModel{}
    |> Systems.Account.Auth.Apple.UserModel.changeset(userinfo)
  ...
  def get_user_by_sub(sub) do
    from(u in User,
      where:
        u.id in subquery(
          from(sc in Systems.Account.Auth.Apple.UserModel,
            where: sc.sub == ^sub,
            select: sc.user_id
          )
        )
  ...
  def register_user(attrs) do
    ...
    with {:ok, apple_user} <-
           %Systems.Account.Auth.Apple.UserModel{}
           |> Systems.Account.Auth.Apple.UserModel.changeset(attrs)
           |> Ecto.Changeset.put_assoc(:user, user)
           |> Repo.insert() do
  ...
  defmacro routes(otp_app) do
    quote bind_quoted: [otp_app: otp_app] do
      ...
      scope "/", Systems.Account.Auth.Apple do
