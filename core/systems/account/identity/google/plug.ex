defmodule Systems.Account.Identity.Google.PlugUtils do
  def config(otp_app) do
    Application.get_env(otp_app, Systems.Account.Identity.Google)
  end
...
defmodule Systems.Account.Identity.Google.AuthorizePlug do
  import Plug.Conn
  import Systems.Account.Identity.Google.PlugUtils
...
defmodule Systems.Account.Identity.Google.CallbackPlug do
  import Plug.Conn
  import Systems.Account.Identity.Google.PlugUtils
  require Logger
  ...
  defp log_session_not_found(conn) do
    Logger.error("[Systems.Account.Identity.Google] OAuth callback without session state",
  ...
  defp redirect_with_error(conn, error) do
    conn
    |> put_flash(:error, Systems.Account.Identity.SSOHelpers.error_message(error))
  ...
  defp authenticate(conn, otp_app, session_params) do
    ...
    case Systems.Account.Identity.authenticate_or_transfer(
      ...
      {:transfer, user} ->
        ...
        |> redirect(to: ~p"/auth/google/transfer")
