defmodule Systems.Account.Identity.Google do
  @moduledoc """
  Google identity provider. Implements `Systems.Account.Identity.Provider` — the
  ...
  `Systems.Account.Identity.authenticate/3`; the orchestrator merges it on top of
  ...
  def get(%User{id: id}) do
    Repo.get_by(Systems.Account.Identity.Google.UserModel, user_id: id)
  end

  @impl Systems.Account.Identity.Provider
  def attach(%User{} = user, userinfo) when is_map(userinfo) do
    %Systems.Account.Identity.Google.UserModel{}
    |> Systems.Account.Identity.Google.UserModel.changeset(userinfo)
  ...
  defmacro routes(otp_app) do
    quote bind_quoted: [otp_app: otp_app] do
      ...
      scope "/", Systems.Account.Identity.Google do
