defmodule Systems.Advert.CreateForm do
  ...
  def update(%{id: id, user: user, target: target}, socket) do
    title = dgettext("eyra-advert", "advert.create.title")
    ...
    |> assign(
      buttons: [
        %{
          ...
          face: %{
            type: :primary,
            label: dgettext("eyra-advert", "advert.create.proceed.button")
          }
        },
        %{
  ...
  def handle_event(
    ...
    advert = create_advert(socket, selected_template, selected_pool)
    {:noreply, socket |> redirect_to(advert.id)}
  end
  ...
  defp redirect_to(%{assigns: %{target: target}} = socket, advert_id) do
    update_target(target, %{module: __MODULE__, action: %{redirect_to: advert_id}})
    socket
  end
  ...
  defp create_advert(
    ...
    socket |> create_advert(template, pool, budget)
  end

  defp create_advert(%{assigns: %{user: user}} = _socket, template, pool, budget) do
    title = dgettext("eyra-dashboard", "default.study.title")
    Advert.Assembly.create(template, user, title, pool, budget)
  end
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <Text.form_field_label id={:template_label}>
        <%= dgettext("eyra-advert", "advert.create.template.label") %>
      ...
      <Text.form_field_label id={:pool_label}>
        <%= dgettext("eyra-advert", "advert.create.pool.label") %>
      </Text.form_field_label>
      <.spacing value="XXS" />
      <.live_component
        module={Selector}
