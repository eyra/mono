defmodule Systems.Advert.ContentPage do
  ...
  use CoreWeb.Layouts.Workspace.Component, :advert
  ...
  def mount(%{"id" => id} = params, _, socket) do
    ...
    model = Advert.Public.get!(String.to_integer(id), Advert.Model.preload_graph(:down))
  ...
  def handle_event(
        "delete_confirm",
        _params,
        %{assigns: %{vm: %{id: advert_id}, current_user: user}} = socket
      ) do
  ...
  def handle_event(
        "submit",
        _params,
        %{assigns: %{vm: %{id: advert_id, submission: submission}}} = socket
      ) do
    socket =
      if Advert.Public.ready?(advert_id) do
