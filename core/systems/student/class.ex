defmodule Systems.Student.Class do
  ...
  def active?(code, active_codes) when is_binary(code) and is_list(active_codes) do
    Enum.member?(active_codes, code)
  end

  def active?(code, active_codes) when is_atom(code) and is_list(active_codes) do
    active?(Atom.to_string(code), active_codes)
  end

  def code(identifier), do: Identifier.to_string(identifier)
