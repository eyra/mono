defmodule Systems.Home.View do
  ...
  defp render_intro(assigns) do
    ...
    <section class="py-6 sm:py-12 bg-white">
      <div class="flex flex-col gap-8 md:flex-row justify-between sm:mt-12 md:pl-14">
        <div class="w-full px-6 sm:px-0 sm:text-center md:text-left md:pr-12 xl:pr-0">
          <h1 class="text-title3 sm:text-title1 font-title1 font-bold text-grey1">
            What's Next?
          </h1>
          <p class="mt-6 text-xl sm:text-2xl font-intro leading-7">
  ...
  defp render_steps(assigns) do
    ~H"""
    <section class="py-6 sm:py-16 sm:px-6">
        <h2 class="text-title3 font-title3 pl-6 text-grey-1">
          How to get started?
        </h2>
        <div class="mt-6 justify-around w-full flex flex-col md:flex-row gap-8 lg:gap-16 px-6">
          <%= render_step_card(%{
  ...
  defp render_step_card(assigns) do
    ...
    <div class="bg-white w-full rounded overflow-hidden drop-shadow-lg">
      <img
        class="w-full h-32 sm:h-46 lg:min-h-80 2xl:min-h-80 object-cover"
        src={@image}
        alt={@title}
      />
      <div class="p-4 lg:p-6">
        ...
        <p class="mt-4 text-lg sm:text-introdesktop ">
          <%= @description %>
        </p>
  ...
  defp render_available_services(assigns) do
    ~H"""
    <section class="h-full pt-6 sm:pt-12 px-6 bg-white">
      <div class="sm:px-12 flex-col gap-10 sm:flex-row justify-start items-center md:gap-10">
          <img class="hidden sm:block h-64" src={~p"/images/landing_page/services-drawing.png"}/>
          <div class="flex flex-col justify-start items-start sm:pr-8 sm:pr-20">
              <div class="pb-6 sm:pb-8 text-title4 font-title4 sm:text-title1 sm:font-title1 text-grey1">
                Available services
              </div>
              <div class="pb-6 flex flex-col justify-start items-start">
                  <div class="pb-6 justify-center text-xl sm:text-2xl font-intro leading-7">
  ...
  defp render_video(assigns) do
    ~H"""
    <section class="h-full pt-8 sm:pt-24 px-6 bg-white">
        <h1 class="text-title3 font-title3 sm:text-title1 sm:font-title1 text-center text-grey1">
          Wil je weten hoe het werkt?
        </h1>
        <div class="flex justify-center pt-6">
          <iframe
            class="h-1/2 sm:h-[60vh] w-full rounded-lg mt-6 shadow"
