defmodule Systems.Campaign.ModelTest do
  ...
  describe "campaigns" do
    ...
    test "list/1 returns all campaigns" do
      ...
      assert Campaign.Public.list() |> Enum.find(&(&1.id == campaign.id))
    end
    ...
    test "list/1 allows excluding a list of ids" do
      ...
      campaign_ids =
        Campaign.Public.list(exclude: [excluded_campaign.id])
        |> Enum.map(& &1.id)
        |> MapSet.new()
    ...
    test "get!/1 returns the campaign with given id" do
      campaign = Factories.insert!(:campaign)
      assert Campaign.Public.get!(campaign.id) != nil
    end
    ...
    test "delete/1 deletes the campaign" do
      campaign = Factories.insert!(:campaign)
      assert {:ok, _} = Campaign.Public.delete(campaign.id)
      assert_raise Ecto.NoResultsError, fn -> Campaign.Public.get!(campaign.id) end
    end

    test "change/1 returns a campaign changeset" do
      campaign = Factories.insert!(:campaign)
      assert %Ecto.Changeset{} = Campaign.Public.change(campaign)
    end
    ...
    test "add_owner!/2 grants a user ownership over a campaign" do
      ...
      Campaign.Public.add_owner!(campaign, researcher_2)
      # The second researcher is now an owner of the campaign
      assert Campaign.Public.list_owned_campaigns(researcher_2) |> Enum.map(& &1.id) == [
               campaign.id
             ]
    end
    ...
    test "assign_owners/2 adds or removes a users ownership of a campaign" do
      ...
      # The second researcher is not the owner of the campaign
      assert Campaign.Public.list_owned_campaigns(researcher_2) == []
      Campaign.Public.assign_owners(campaign, [researcher_2])
      # The second researcher is now an owner of the campaign
      assert Campaign.Public.list_owned_campaigns(researcher_2) |> Enum.map(& &1.id) == [
               campaign.id
             ]
      ...
      assert Campaign.Public.list_owned_campaigns(researcher_1) == []
    end
    ...
    test "list_owners/1 returns all users with ownership permission on the campaign" do
      ...
      campaign = Factories.insert!(:campaign)
      :ok = Authorization.assign_role(researcher_1, campaign, :owner)
      assert Campaign.Public.list_owners(campaign) |> Enum.map(& &1.id) == [researcher_1.id]
      Campaign.Public.assign_owners(campaign, [researcher_2])
      assert Campaign.Public.list_owners(campaign) |> Enum.map(& &1.id) == [researcher_2.id]
    end
  end
end
