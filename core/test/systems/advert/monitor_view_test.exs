defmodule Systems.Advert.MonitorViewTest do
  ...
  describe "show content page for advert" do
    setup [:login_as_researcher]

    test "No member applied", %{conn: %{assigns: %{current_user: user}} = conn} do
      %{id: id} = Advert.Factories.create_advert(user, :accepted, 1)

      {:ok, _view, html} = live(conn, Routes.live_path(conn, Advert.ContentPage, id))
    ...
    test "Member applied but expired and not completed", %{
      ...
      %{id: id, assignment: %{crew: crew}} = Advert.Factories.create_advert(user, :accepted, 1)
    ...
    test "Member applied but expired and not completed: accept ", %{
      ...
      {:ok, view, html} = live(conn, Routes.live_path(conn, Advert.ContentPage, id))
    ...
    test "Member applied but expired and not completed: accept_all", %{
      ...
      %{id: id, assignment: %{crew: crew}} = Advert.Factories.create_advert(user, :accepted, 2)
