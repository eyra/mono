defmodule Systems.Account.Auth.Apple.Test do
  ...
  describe "get_user_by_sub/1" do
    test "get a user by their subject id" do
      ...
      Repo.insert!(%Systems.Account.Auth.Apple.UserModel{sub: "test", user: user})

      loaded_user = Systems.Account.Auth.Apple.get_user_by_sub("test")
  ...
  describe "register_apple_user/1" do
    test "creates a user with a profile" do
      for middle_name? <- [false, true] do
        ...
        {:ok, apple_user} = Systems.Account.Auth.Apple.register_user(sso_info)
