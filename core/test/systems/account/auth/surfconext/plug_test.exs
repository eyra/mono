defmodule Systems.Account.Auth.Surfconext.FakeOIDC do
...
defmodule Systems.Account.Auth.Surfconext.AuthorizePlug.Test do
  use ExUnit.Case, async: false
  import Plug.Test
  import Plug.Conn
  alias Systems.Account.Auth.Surfconext.AuthorizePlug

  describe "call/1" do
    test "redirects to Surfconext login page" do
      domain = Faker.Internet.domain_name()

      Application.put_env(:test, Systems.Account.Auth.Surfconext,
        client_id: domain,
...
defmodule Systems.Account.Auth.Surfconext.CallbackController.Test do
  use CoreWeb.ConnCase, async: false

  setup do
    conf = Application.get_env(:core, Systems.Account.Auth.Surfconext, [])

    on_exit(fn ->
      Application.put_env(:core, Systems.Account.Auth.Surfconext, conf)
    end)
    ...
    Application.put_env(:core, Systems.Account.Auth.Surfconext, test_conf)
  ...
  describe "authenticate/1" do
    ...
    test "authenticates an existing user", %{conn: conn, conf: conf} do
      ...
      Systems.Account.Auth.authenticate(Systems.Account.Auth.Surfconext, %{
        "sub" => "test",
        "email" => email,
      ...
      Application.put_env(
        :core,
        Systems.Account.Auth.Surfconext,
    ...
    test "updates an existing student", %{conn: conn, conf: conf} do
      ...
      surfconext_user = Systems.Account.Auth.Surfconext.get(user)
    ...
    test "offers account transfer for an existing email/password account", %{
      ...
      assert Systems.Account.Auth.Surfconext.get(reloaded) == nil
    end
