defmodule Systems.Account.Identity.Google.Test do
  ...
  describe "user_attrs/1" do
    test "maps proprietary fields to the normalized Account.User attrs" do
      ...
      attrs = Systems.Account.Identity.Google.user_attrs(userinfo)
  ...
  describe "get/1" do
    test "returns the satellite linked to the user" do
      ...
      satellite =
        Repo.insert!(%Systems.Account.Identity.Google.UserModel{sub: "test", user: user})

      assert Systems.Account.Identity.Google.get(user).id == satellite.id
    end

    test "returns nil when the user has no satellite" do
      user = Factories.insert!(:member)

      assert Systems.Account.Identity.Google.get(user) == nil
    end
  end
  ...
  describe "attach/2" do
    test "inserts a satellite row linked to the user" do
      ...
      {:ok, satellite} = Systems.Account.Identity.Google.attach(user, userinfo)
  ...
  describe "refresh/2" do
    test "replaces userinfo on the existing satellite without touching sub or user" do
      ...
      {:ok, before} = Systems.Account.Identity.Google.attach(user, original)
      ...
      refreshed = Systems.Account.Identity.Google.refresh(user, updated)
