defmodule Systems.Account.Identity.Google.FakeGoogle do
  @sub Faker.UUID.v4()
...
defmodule Systems.Account.Identity.Google.AuthorizePlug.Test do
  use ExUnit.Case, async: true
  import Plug.Test
  import Plug.Conn
  alias Systems.Account.Identity.Google.AuthorizePlug

  describe "call/1" do
    setup do
      domain = Faker.Internet.domain_name()

      Application.put_env(:test, Systems.Account.Identity.Google,
        client_id: domain,
        client_secret: Faker.Lorem.sentence(),
        site: "https://connect.test.google_sign_in.nl",
        redirect_uri: "https://#{domain}/google_sign_in/auth",
        google_module: Systems.Account.Identity.Google.FakeGoogle
      )
...
defmodule Systems.Account.Identity.Google.CallbackPlug.Test do
  use ExUnit.Case, async: false
  use Core.DataCase
  import Plug.Test
  import ExUnit.CaptureLog
  use Core.FeatureFlags.Test
  alias Systems.Account.Identity.Google.CallbackPlug
  ...
  describe "call/1" do
    ...
    test "authenticates an existing user" do
      email = Systems.Account.Identity.Google.FakeGoogle.email()

      given_name = Faker.Person.first_name()
      family_name = Faker.Person.last_name()

      Systems.Account.Identity.authenticate(
        Systems.Account.Identity.Google,
        %{
          "sub" => Systems.Account.Identity.Google.FakeGoogle.sub(),
          "name" => "#{given_name} #{family_name}",
          "email" => email,
          "email_verified" => true,
          "given_name" => given_name,
    ...
    test "offers transfer for an existing account without a Google satellite" do
      email = Systems.Account.Identity.Google.FakeGoogle.email()
      Core.Factories.insert!(:creator, %{email: email})
      ...
      assert Plug.Conn.get_resp_header(conn, "location") == ["/auth/google/transfer"]
    end
    ...
    test "allow admin when member login is disabled" do
      Application.put_env(
        :core,
        :admins,
        Systems.Admin.Public.compile([Systems.Account.Identity.Google.FakeGoogle.email()])
      )
      ...
      assert user.email == Systems.Account.Identity.Google.FakeGoogle.email()
    end
    ...
    test "redirects to signin and logs error when session is missing" do
      ...
      assert log =~ "[error]"
      assert log =~ "[Systems.Account.Identity.Google] OAuth callback without session state"
    end
  end
end
