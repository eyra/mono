defmodule Systems.Survey.PublicTest do
  use Core.DataCase
  ...
  describe "survey_tools" do
    ...
    test "list_survey_tools/0 returns all survey_tools" do
      ...
      assert Survey.Public.list_survey_tools()
             |> Enum.map(& &1.id)
             |> MapSet.new()
    ...
    test "get_survey_tool!/1 returns the survey_tool with given id" do
      survey_tool = Factories.insert!(:survey_tool)
      assert Survey.Public.get_survey_tool!(survey_tool.id).id == survey_tool.id
    end

    test "create_tool/1 with valid data creates a survey_tool" do
      auth_node = Factories.insert!(:auth_node)

      assert {:ok, %Survey.ToolModel{} = _survey_tool} = Survey.Public.create_tool(%{}, auth_node)
    end
    ...
    test "update_survey_tool/2 with valid data updates the survey_tool" do
      ...
      assert {:ok, %{tool: survey_tool}} =
               Survey.Public.update_survey_tool(survey_tool, :auto_save, @update_attrs)
    ...
    test "delete_survey_tool/1 deletes the survey_tool" do
      survey_tool = Factories.insert!(:survey_tool)
      assert {:ok, %{}} = Survey.Public.delete_survey_tool(survey_tool)
      assert_raise Ecto.NoResultsError, fn -> Survey.Public.get_survey_tool!(survey_tool.id) end
    end

    test "change_survey_tool/1 returns a survey_tool changeset" do
      survey_tool = Factories.insert!(:survey_tool)
      assert %Ecto.Changeset{} = Survey.Public.change_survey_tool(survey_tool, :mount)
    end
  end
end
