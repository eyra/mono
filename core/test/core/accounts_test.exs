defmodule Core.AccountsTest do
  ...
  describe "get_user_by_email/1" do
    ...
    test "test Jason.Encoder" do
      ...
      assert Jason.encode!(user_db) ==
               "{\"id\":#{user.id},\"email\":\"#{user.email}\",\"displayname\":\"#{user.displayname}\",\"visited_pages\":null,\"researcher\":null,\"student\":null,\"coordinator\":null}"
    end
  end
  ...
  describe "get_user_by_email_and_password/2" do
    ...
    test "does not return the user if the password is not valid" do
      user = Factories.insert!(:member)
      refute Accounts.get_user_by_email_and_password(user.email, Factories.valid_user_password())
    end
  ...
  describe "register_user/1" do
    ...
    test "registers users with a hashed password" do
      ...
      {:ok, user} =
        Accounts.register_user(%{email: email, password: Factories.valid_user_password()})
  ...
  describe "apply_user_email/3" do
    ...
    test "requires email to change", %{user: user} do
      {:error, changeset} = Accounts.apply_user_email(user, Factories.valid_user_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{user: user} do
      {:error, changeset} =
        Accounts.apply_user_email(user, Factories.valid_user_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.apply_user_email(user, Factories.valid_user_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.apply_user_email(user, Factories.valid_user_password(), %{email: email})
  ...
  describe "update_user_password/3" do
    ...
    test "validates password", %{user: user} do
      {:error, changeset} =
        Accounts.update_user_password(user, Factories.valid_user_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{user: user} do
      ...
      {:error, changeset} =
        Accounts.update_user_password(user, Factories.valid_user_password(), %{password: too_long})
  ...
  describe "reset_user_password/2" do
    ...
    test "deletes all tokens for the given user", %{user: user} do
      _ = Accounts.generate_user_session_token(user)
      {:ok, _} = Accounts.reset_user_password(user, %{password: Factories.valid_user_password()})
      refute Repo.get_by(UserToken, user_id: user.id)
    end
  end
  ...
  describe "profile" do
    ...
    test "get_user_profile!/1 returns an empty user profile when it does not yet exist", %{
      ...
      assert Accounts.get_profile(user.id) |> Map.get(:user_id) == user.id
    end
    ...
    test "get_user_profile!/1 returns the same user profile for the user with the given id", %{
      ...
      assert Accounts.get_profile(user.id) |> Map.get(:id) == profile.id
    end

    test "get_user_profile!/1 returns the user profile when given a user", %{
      user: user
    } do
      assert Accounts.get_profile(user) == Accounts.get_profile(user.id)
    end
