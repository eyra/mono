  ...
  describe "survey_tools" do
    ...
    test "create_survey_tool/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} =
               Tools.create_survey_tool(@invalid_attrs, Factories.insert!(:study))
    end

    test "update_survey_tool/2 with valid data updates the survey_tool" do
      survey_tool = Factories.insert!(:survey_tool)

      assert {:ok, %Tool{} = survey_tool} = Tools.update_survey_tool(survey_tool, @update_attrs)
    ...
    test "update_survey_tool/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} = Tools.update_survey_tool(survey_tool, @invalid_attrs)

      assert survey_tool.id == Tools.get_survey_tool!(survey_tool.id).id
    end

    test "delete_survey_tool/1 deletes the survey_tool" do
      survey_tool = Factories.insert!(:survey_tool)
      assert {:ok, %Tool{}} = Tools.delete_survey_tool(survey_tool)
    ...
    test "get_task/2 returns a task when available" do
      ...
      assert Tools.get_task(survey_tool_task.survey_tool, survey_tool_task.user)
