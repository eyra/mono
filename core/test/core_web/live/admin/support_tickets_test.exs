defmodule CoreWeb.Admin.SupportTicketsTest do
  use CoreWeb.ConnCase, async: false
  import Phoenix.LiveViewTest
  alias Core.Factories
  alias CoreWeb.Admin.SupportTickets

  describe "require admin role" do
    setup [:login_as_member]

    test "deny access to non-admin", %{conn: conn} do
      {:ok, _view, html} = live(conn, Routes.live_path(conn, SupportTickets))
  ...
  describe "helpdesk tickets" do
    ...
    test "closing a ticket removes it", %{conn: conn} do
      ticket = Factories.insert!(:helpdesk_ticket)
      {:ok, view, _html} = live(conn, Routes.live_path(conn, SupportTickets))
