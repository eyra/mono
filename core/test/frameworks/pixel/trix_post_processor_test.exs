defmodule Frameworks.Pixel.TrixPostProcessorTest do
  ...
  describe "add_target_blank/1" do
    test "adds target=_blank to <a> tags without target attribute" do
      ...
      expected = ~s(<a href="http://example.com" target="_blank">Example</a>)
    ...
    test "does not modify <a> tags with existing target attribute" do
      html_content = ~s(<a href="http://example.com" target="_self">Example</a>)
      expected = ~s(<a href="http://example.com" target="_self">Example</a>)
      assert TrixPostProcessor.add_target_blank(html_content) == expected
    end

    test "adds target=_blank to multiple <a> tags without target attribute" do
      html_content =
        ~s(<a href="http://example1.com">Example1</a> <a href="http://example2.com">Example2</a>)

      expected =
        ~s(<a href="http://example1.com" target="_blank">Example1</a> <a href="http://example2.com" target="_blank">Example2</a>)
    ...
    test "handles nested <a> tags correctly" do
      ...
      expected = ~s(<div><a href="http://example.com" target="_blank">Example</a></div>)
      assert TrixPostProcessor.add_target_blank(html_content) == expected
    end
